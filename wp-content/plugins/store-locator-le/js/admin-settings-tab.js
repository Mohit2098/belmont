...
let slp_setting_helper = function () {
    ...
    this.initialize = function () {
        jQuery('div[data-related_to]').hover(
            SLP_ADMIN.helper.related_on,
            SLP_ADMIN.helper.related_off
        );
...
let SLP_Admin_Plugin_Style = function () {
    ...
    this.add_more_style = function (data) {
        ...
        jQuery(data).each(function () {
            ...
            jQuery(vision_list)
                ...
                .on('click', SLP_Admin_Settings_Help.PluginStyle.set_active_style);
        });
        SLP_Admin_Settings_Help.PluginStyle.scroll_in_progress = false;
    };
    ...
    this.set_active_style = function () {
        ...
        jQuery('#options_nojs\\[style_id\\]').val(
            jQuery(this).attr('data-post_id')
        );

        let active_div = jQuery('div.active');
        jQuery(active_div)
            .find('.button-secondary')
            .removeClass('customize')
            .addClass('activate');
        jQuery(active_div).removeClass('active');

        jQuery('.' + clicked_theme).addClass('active');

        active_div = jQuery('div.active');
        jQuery(active_div)
            .find('.button-secondary')
            .addClass('customize')
            .text(jQuery('#activating_text').val());
