		...
		public function set_QueryParams() {
			...
			if ( ! $this->formdata_set ) {
				...
				if ( $hasFormData && ( $slpNonceOK || $wpRESTNonceOK ) ) {
					// cannot run wp_kses_post_deep on this -- it fucks query parms converting x=1&y=2 into x=1&amp;y=2
			...
			foreach ( $this->query_params_valid as $key ) {
				...
				// if ($this->query_params[$key]  !== $_REQUEST[$key]) $mungedQueryParams[$key] = array($_REQUEST[$key], $this->query_params[$key]);
			}
			// $x = $mungedQueryParams;
