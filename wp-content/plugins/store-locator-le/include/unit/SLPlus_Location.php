...
class SLPlus_Location {
	...
	private $db;
	private $delay = self::StartingDelay;
	...
	private $legacy_field_map = array(
		'sl_id' => 'id',
		'sl_store' => 'store',
		'sl_address' => 'address',
		...
		'sl_image' => 'image',
		'sl_private' => 'private',
		'sl_neat_title' => 'neat_title',
		'sl_linked_postid' => 'linked_postid',
		'sl_pages_url' => 'pages_url',
		'sl_pages_on' => 'pages_on',
		'sl_option_value' => 'option_value',
		'sl_lastupdated' => 'lastupdated',
		'sl_initial_distance' => 'initial_distance'
	);
	...
	public function add_to_database( $locationData, $duplicates_handling = 'none', $skipGeocode = false ) {
		...
		// Write to disk, return not_updated if no changes made to DB.
		//
		if ( ! $this->MakePersistentIfChanged() ) {
		...
		if ( $update_ok ) {
			...
			if ( $touched_pageID != $this->linked_postid ) {
				...
				foreach ( $this->dbFields as $property ) {
					$key = 'slp_location_' . $property;
					add_post_meta( $this->linked_postid, $key, $this->$property, true );
				}

				if ( $MakePersistent ) {
					$this->MakePersistentIfChanged();
				}
			}
	...
	public function do_geocoding( $address = null ) {
		...
		if ( ! empty( $address ) ) {
			...
			//
			$json = SLP_Location_Utilities::get_instance()->geocode( array( 'address' => $address ) );
			if ( ! empty( $json ) ) {
				$status = property_exists( $json , 'status' ) ? $json->status : 'ERROR' ;
				switch ( $status ) {
					...
					case 'OVER_QUERY_LIMIT':
						$errorMessage .= sprintf( __( "Address %s (%d in current series) hit the Google query limit or your API key is missing.\n", 'store-locator-le' ),
								$address,
								$this->count
						                 ) . '<br/>';
						if (  property_exists( $json , 'error_message' ) && ! empty( $json->error_message ) ) {
							$errorMessage .= $json->error_message . '<br/>';
						}
					...
					//
					case 'ZERO_RESULTS':
						$errorMessage .= sprintf( __( 'Address #%d : %s failed to geocode.', 'store-locator-le' ),
								$this->id,
								$address
						                 ) . "<br />\n";
					...
					default:
						$errorMessage .=
							sprintf( __( 'Address #%d : %s <span color=red>failed to geocode</span>.', 'store-locator-le' ),
								$this->id,
								$address ) .
							"<br/>\n" .
						...
						if (  property_exists( $json , 'error_message' ) && ! empty( $json->error_message ) ) {
							...
							$errorMessage .= sprintf( __( 'Received data %s.', 'store-locator-le' ),
		...
		if ( $errorMessage != '' ) {
			...
			$this->slplus->notifications->add_notice(
				9,
				sprintf(
					__( 'Google thinks %s is at <a href="%s" target="_blank">lat: %s lng %s</a>', 'store-locator-le' ),
					$address,
	...
	public function delete( $id = null, $all = false ) {
		...
		//
		if ( (int) $this->id < 0 ) {
			return new WP_Error( 'slp_invalid_id', $this->slplus->Text->get_text_string( array(
				'label',
				'slp_invalid_id'
			) ), array( 'status' => 404 ) );
		}
	...
	public function delete_page_links() {
		if ( ! empty( $this->linked_postid ) ) $this->dataChanged = true;
		if ( ! empty( $this->pages_url ) ) $this->dataChanged = true;
	...
	public function get_display_type( $property ) {
		if ( !array_key_exists( $property , $this->display_types ) ) {
			if ( $property === 'image' ) {
				$this->display_types[ $property ] = 'image';
			} elseif ( $property === 'private' ) {
				$this->display_types[ $property ] = 'checkbox';
			} elseif ( $this->is_base_field( $property ) ) {
				$this->display_types[ $property ] = null;
			} else {
				$this->display_types[ $property ] = $this->slplus->database->extension->get_option( $property, 'display_type' );
			}
		}

		return $this->display_types[ $property ];
	}
	...
	public function get_property_name( $field_slug ) {
		if ( empty( $this->property_names[ $field_slug ] ) ) {
			if ( strpos( $field_slug , 'sl_' ) !== 0 ) {
				$this->property_names[ $field_slug ] = $field_slug;
			} else {
				$this->property_names[ $field_slug ] = preg_replace( '/^sl_/', '', $field_slug );
			}
		}
		return $this->property_names[ $field_slug ];
	}
	...
	public function get( $output = OBJECT ) {
		...
		$slfields = array_reduce( $this->dbFields, array( $this, 'mapPropertyToField' ) );
		$data     = array_merge( $slfields, $this->exdata );
	 ...
	 * @return mixed|string
	 */
	public function get_property_using_fieldname( $fieldname ) {
		$property =  ! empty ( $this->legacy_field_map[ $fieldname ] ) ? $this->legacy_field_map[ $fieldname ] : $fieldname;
	...
	private function set_LatLong( $lat, $lng ) {
		...
		// Do not calculate initial distance if this was set via a db load
		if ( $this->data_is_loading ) {
			return;
		}

		// Always calc initial distance if blank.
		if ( empty( $this->initial_distance ) ) {
			$this->calculate_initial_distance();
			if (! empty( $this->initial_distance ) ) $this->dataChanged = true;
			$distance_changed = true;
		}

		// If lat/lng changed, dataChanged is true
		// If distance was not already re-calculated above, calculate the new distance now.
		if ( $lat_or_lng_changed ) {
			if ( ! $distance_changed ) $this->calculate_initial_distance();
			$this->dataChanged = true;
		}
	}
	 ...
	 */
	public function MakePersistentIfChanged() {
	...
	function isvalid_ID( $id = null, $request_param = null ) {
        if (!empty($_REQUEST['security']) && !check_admin_referer('slp_ajax', 'security')) return false;
	...
	public function update_Attributes( $newAttributes ) {
		if ( is_array( $newAttributes ) ) {
			$original_attributes = $this->attributes;
			$this->attributes  =
				is_array( $this->attributes ) ?
					array_merge( $this->attributes, $newAttributes ) :
					$newAttributes;

			if ( $original_attributes !== $this->attributes ) {
				$this->dataChanged = true;
			}
		}
	}
