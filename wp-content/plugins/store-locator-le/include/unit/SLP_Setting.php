...
class SLP_Setting extends SLPlus_BaseClass_Object {
	...
	public function filter_wp_kses_allowed_admin_tags( $allowed_tags ) {
		...
		//$allowed_tags['a']  = $standard_attributes;
	...
	protected function wrap_in_default_html() {
		//        $origContent = preg_replace('/\s+/', ' ', $this->content);
		//        $origContent = preg_replace('/\s>/','>',$origContent);
		//        $origContent = preg_replace('/<br\/>/','<br />',$origContent);
