<?php
	...
	class SLP_Settings_Group extends SLPlus_BaseClass_Object {
		...
		private function display_header() {
			?>
			<div id='wpcsl_settings_group-<?php echo  esc_attr($this->slug) ?>' class='settings-group ui-accordion-content-active' >
                <?php if ( ! empty( $this->header ) ) : ?>
				<h4 class='settings-header ui-accordion-header'><?php echo  wp_kses_post( $this->header) ?></h4>
                <?php endif; ?>
				<div class='inside'>
					<?php if ( ! empty( $this->intro ) ) : ?>
						<div class='section_column_intro' id='wpcsl_settings_group_intro-<?php echo  esc_attr($this->slug) ?>'>
							<?php echo  wp_kses_post($this->intro) ?>
						</div>
					<?php endif; ?>
			<?php
		}
