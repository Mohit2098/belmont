...
if ( ! class_exists( 'SLP_Settings_Section' ) ) {
	...
	class SLP_Settings_Section extends SLPlus_BaseClass_Object {
			...
			if ( isset( $this->groups ) ) {
				foreach ( $this->groups as $group ) {
					if ( ! empty( $group->div_group ) && ( $group->div_group != $this->current_div_group ) ) {
						...
						?>
						<div class='<?php echo esc_attr($group->div_group) ?>'>
						<?php
		...
		private function header() {
            ...
            ?>
			<div id='wpcsl-option-<?php echo esc_attr($friendlyDiv) ?>' class='<?php echo esc_attr($groupClass) ?> subtab_<?php echo esc_attr($friendlyDiv) ?> subtab settings'>
			    <?php echo  wp_kses_post($this->opening_html) ?>
                <?php
                if ( $this->innerdiv ) {
		...
		private function footer() {
			...
			print wp_kses_post($this->closing_html);
			echo '</div>';
		}
