...
class SLP_Admin_Locations_Add extends SLPlus_BaseClass_Object {
	...
	public function initialize() {
		...
		$this->settings                   = SLP_Admin_Locations::get_instance()->settings;
		...
		<v-app>
			...
			<v-dialog v-model="show_add_dialog" hide-overlay persistent scrollable eager>
				<v-card>
					<v-card-title>
						<span class="headline">{{location_manager.text.form_title}}</span>
					</v-card-title>
					<v-divider></v-divider>
					<v-card-text>
						<v-container>
							<v-row class="d-flex flex-row-reverse">
								<v-btn v-on:click.native="submit_form" color="primary">{{location_manager.text.save}}</v-btn>              
			            		<v-btn v-on:click.native="close_add_dialog" class="mr-2">{{location_manager.text.cancel}}</v-btn>
							</v-row>
							<v-row>
HTML;
		...
		$this->section_params['closing_html'] = <<< HTML_END
					...
					<v-card-actions class="mb-4">	
			            <v-btn v-on:click.native="close_add_dialog" class="ml-2 mr-2">{{location_manager.text.cancel}}</v-btn>
			            <v-btn v-on:click.native="submit_form" color="primary" >{{location_manager.text.save}}</v-btn>              
	                </v-card-actions>
				</v-card>				
			</v-dialog>
...
HTML_END;
	...
	public function build_interface() {
		...
		add_action( 'slp_modify_location_add_form', array(
			$this,
			'map'
		), 20 ); // Allows other extensions to remove this.
	...
	private function extended_data_block() {
		SLP_Admin_Locations::get_instance()->set_active_columns();
		SLP_Admin_Locations::get_instance()->filter_active_columns();
		if ( empty( SLP_Admin_Locations::get_instance()->active_columns ) ) {
			return;
		}
		...
		//
		$groups = array();
		foreach ( SLP_Admin_Locations::get_instance()->active_columns as $data_field ) {
	...
	public function map() {
		...
		$this->settings->add_ItemToGroup( array(
			...
			'wrapper'      => false,
			'type'         => 'custom',
			'custom'       => "<div v-show='(location.sl_latitude!==\"\")' data-maptype='google_dynamic' id='edit_location_map' class='location_map'></div>",
		) );
	}
