...
class SLP_Admin_Activation extends SLPlus_BaseClass_Object {
	...
	public function being_deactivated() {
		...
		if ( empty ( $this->slplus->clean['deactivate'] ) ) {
			return false;
		}
		if ( ! check_admin_referer( 'deactivate-plugin_' . $this->slplus->slug ) ) {
			return false;
		}
	...
	private function dbupdater( $sql, $table_name ) {
		...
		/** @var wpdb $wpdb */
		global $wpdb;
		// phpcs:ignore -- this direct db call is required to check the table state
		// phpcs:ignore
		...
		dbDelta( $sql );
		global $EZSQL_ERROR;
		// phpcs:ignore -- this is required to make sure we are catching the DB errors properly, not WP core..
		// if wp had proper hooks or filters in this module it would not be an issue... but they don't so here we are...
		// phpcs:ignore
		$EZSQL_ERROR = array();
	 ...
	 */
	private function iconMapper( $iconFile ) {
	 ...
	 */
	private function install_ExtendedDataTables() {
		global $wpdb;
		$charset_collate = '';
	 ...
	 *
	 */
	private function install_main_table() {
