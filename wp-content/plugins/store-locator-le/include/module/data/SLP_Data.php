...
class SLP_Data extends SLPlus_BaseClass_Object {
    ...
    function get_SQL($commandList) {
        ...
        foreach ($commandList as $command) {
            switch ($command) {
	            ...
	            case 'select_zip':
		            $sqlStatement .= 'SELECT trim(sl_zip) as zip' . $this->from_slp_table;
		            break;
	            ...
	            case 'where_valid_zip':
		            $sqlStatement .= $this->add_where_clause("(sl_zip<>'') AND (sl_zip IS NOT NULL)");
		            break;
                ...
                case 'order_by_country':
                    ...
                    $sqlStatement .= $this->add_order_by_clause();
                    break;
	            ...
	            case 'order_by_zip':
		            $this->extend_order_array('zip ASC');
		            $sqlStatement .= $this->add_order_by_clause();
		            break;
	            ...
	            case 'group_by_country':
                    $sqlStatement .= $this->add_group_by_clause('country');
                    break;

                case 'group_by_state':
                    $sqlStatement .= $this->add_group_by_clause('state');
                    break;

	            case 'group_by_zip':
		            $sqlStatement .= $this->add_group_by_clause('zip');
		            break;
    ...
    function get_Record($commandList, $params = array(), $offset = 0, $type = ARRAY_A, $mode = 'get_row') {
        ...
        } elseif ( $mode === 'get_results' ) {
        	$results = $this->db->get_results( $query , $type );
        } else {
    ...
    public function has_extended_data() {
        ...
        if ( ! $this->is_Extended() ) {
            ...
            global $wpdb;
            // phpcs:ignore data table['name'] var can only come from sanctioned add on plugin code
            // phpcs:ignore
