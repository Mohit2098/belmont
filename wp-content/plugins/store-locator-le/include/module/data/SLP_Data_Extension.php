...
class SLP_Data_Extension extends SLPlus_BaseClass_Object {
	...
	function get_data( $sl_id, $field_id = NULL ) {
        ...
        // phpcs:ignore
		$query = $wpdb->prepare( "select * from {$this->data_table['name']} where sl_id = %s", intval($sl_id) );
        // phpcs:ignore we don't want cached data
        // phpcs:ignore
	...
	function update_data( $sl_id, $data ) {
		...
		if ( empty ( $currentData ) ) {
			...
			$data[ 'sl_id' ] = $sl_id;
            // phpcs:ignore direct data call without caching is what we want here
            // phpcs:ignore
	...
	function update_data_table( $params = array() ) {
		...
		if ( is_array( $extended_fields ) && ( count( $extended_fields ) > 0 ) ) {
			...
			dbDelta( $create );
			global $EZSQL_ERROR;
            // phpcs:ignore we need this to be able to catch data struct issues when building our custom table
            // phpcs:ignore
			$EZSQL_ERROR = array();
		}
