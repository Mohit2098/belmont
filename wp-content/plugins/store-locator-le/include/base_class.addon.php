...
if ( ! class_exists( 'SLP_BaseClass_Addon' ) ) {
	...
	class SLP_BaseClass_Addon extends SLP_Object_WithOptions {
		...
		protected function initialize() {
			...
			// slp_init calls \SLP_AddOns::register
		 ...
		 *
		 * @used-by \SLP_BaseClass_Addon::initialize via SLP Filter slp_init_complete
		 *
		 * @uses    \SLP_BaseClass_Addon::createobject_UserInterface via WP Filter wp_enqueue_scripts
		 */
		public function slp_init() {
			...
			if ( ! empty( $this->userinterface_class_name ) ) {
				add_action( 'template_redirect', array( $this, 'createobject_UserInterface' ) );
			}
		...
		private function create_object_option_manager() {
			if ( ! isset( $this->WPOption_Manager ) ) {
				/** @noinspection PhpIncludeInspection */
				require_once( SLPLUS_PLUGINDIR . 'include/module/wpoption/SLP_WPOption_Manager.php' );
		...
		public function createobject_UserInterface() {
			if ( ! isset( $this->userinterface ) ) {
				...
				if ( is_readable( $this->dir . 'include/class.userinterface.php' ) ) {
					...
					$this->userinterface = new $this->userinterface_class_name( array(
						'addon'  => $this,
						'slplus' => $this->slplus
					) );
		...
		private function VersionCheck( $params ) {
			...
			if ( version_compare( SLPLUS_VERSION, $params['min_required_version'], '<' ) ) {
				if ( is_admin() ) {
					...
					SLP_Admin_Helper::get_instance()->add_wp_admin_notification( sprintf( __( '%s requires Store Locator Plus® version %s. ', 'store-locator-le' ), $this->addon->name, $params['min_required_version'] ) . __( 'This add-on has been deactivated', 'store-locator-le' ) );
				}
