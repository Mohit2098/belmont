...
class SLP_Experience_Widget extends SLPlus_BaseClass_Object {
    ...
    function enqueue_javascript() {
        ...
        if ( file_exists( $this->addon->dir . 'include/widgets/slp_widgets.js' ) ) {
            ...
            wp_add_inline_script(
                $this->addon->slug . '_widgets' ,
                'const slp_experience = ' . wp_json_encode($this->js_settings)
            );
        }
    }
    ...
    function set_SearchCenter($currentVal) {
        if ( ( ! empty( $_REQUEST['widget_address'] ) ) ) {
            return stripslashes($_REQUEST['widget_address']);
        }

	    if ( ! empty( $_REQUEST['address'] ) &&  SLP_Experience_UI::get_instance()->is_address_passed_by_URL() ) {
	    	return stripslashes( $_REQUEST[ 'address' ] );
	    }
