...
class SLP_Experience_AJAX extends SLP_BaseClass_AJAX {
	...
	private function add_action_handlers() {
		...
		add_action( 'wp_ajax_nopriv_slp_list_location_zips', array( $this, 'list_location_zips' ) );
	}
	...
	public function modify_having_clause( $having_clauses ) {
		...
		if ( $_REQUEST['action'] === 'csl_ajax_search' ) {
			...
			if ( $this->radius_behavior_ignores_radius() ) {
				$having_clauses   = array_filter( $having_clauses, array(
					$this->slplus->AJAX,
	...
	public function filter_JSONP_SearchByStore( $currentFilters ) {
		...
		$posted_name = preg_replace( '/^\s+(.*?)/', '$1', $_REQUEST['store_name'] );
	...
	public function filter_JSONP_SearchFilters_W( $where ) {
		...
		if ( ! $this->addon->widget->is_initial_widget_search( $this->formdata ) ) {
			return $where;
		}
		if ( ! isset( $this->formdata['slp_widget'] ) ) {
			return $where;
		}
	...
	private function get_email_by_slid( $id ) {
		if ( ! $this->slplus->currentLocation->isvalid_ID( $id ) ) {
			return '';
		}
	...
	public function list_location_zips() {
		SLP_Experience_Data::get_instance();
		$this->set_autocomplete_vars();
	...
	public function modify_marker( $marker ) {
		...
		if ( ! empty( $marker['email'] ) ) {
			switch ( $this->slplus->SmartOptions->email_link_format->value ) {
		...
		if ( $this->slplus->database->is_Extended() && $this->slplus->database->has_extended_data() ) {
			...
			if ( ! isset( $marker['featured'] ) ) {
				$marker['featured'] = '';
			}
			if ( ! isset( $marker['rank'] ) ) {
				$marker['rank'] = '';
			}
		}
	...
	public function process_popup_email_form() {
		...
		if ( defined( 'MYSLP_VERSION' ) ) {
			$message_headers[] = "From: MySLP Location Popup Email <no-reply@storelocatorplus.com>";
		} else {
			$message_headers[] = "From: {$this->formdata[ 'email_from' ]} <{$this->formdata[ 'email_from' ]}>";
		}
		$message_headers[] = "Reply-To: {$this->formdata[ 'email_from' ]}";
		$message_headers[] = 'Content-Type: text/html; charset=' . get_option( 'blog_charset' );

		if ( wp_mail( $to_email, $this->formdata['email_subject'], $this->formdata['email_message'], $message_headers ) ) {
			die( __( 'email sent to ' . $to_email, 'slp-experience' ) );
		} else {
			die ( __( 'email not sent to ' . $to_email, 'slp-experience' ) );
		}
	}
	...
	private function radius_behavior_ignores_radius() {
		...
		if ( $this->slplus->options_nojs['radius_behavior'] == "ignore_with_blank_addr" && empty( $this->formdata['addressInput'] ) ) {
			return true;
		}
	...
	public function remove_SQL_limit( $query ) {
		...
		if ( $this->is_discrete_widget_search( array( 'state', 'search' ) ) ) {
			...
			if ( $count > 0 ) {
				add_filter( 'slp_ajaxsql_queryparams', array( $this, 'remove_SQL_limit_param' ), 9 );
			}
		}
